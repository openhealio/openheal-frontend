...
export default class BioReport {
  ...
  constructor(bioimpedance, language, name) {
    ...
    this.cat_icw =
      Number(this.bioimpedance.icw) < Number(this.bioimpedance.icw_min)
        ? "under_bad"
        : Number(this.bioimpedance.icw) > Number(this.bioimpedance.icw_max)
        ? "over_bad"
        : "normal";
    this.cat_ecw =
      Number(this.bioimpedance.ecw) < Number(this.bioimpedance.ecw_min)
        ? "under_bad"
        : Number(this.bioimpedance.ecw) > Number(this.bioimpedance.ecw_max)
        ? "over_bad"
        : "normal";
    this.cat_protein =
      Number(this.bioimpedance.protein) < Number(this.bioimpedance.protein_min)
        ? "under_bad"
        : Number(this.bioimpedance.protein) >
          Number(this.bioimpedance.protein_max)
        ? "over_good"
        : "normal";
    this.cat_mineral =
      Number(this.bioimpedance.mineral) < Number(this.bioimpedance.mineral_min)
        ? "under_bad"
        : Number(this.bioimpedance.mineral) >
          Number(this.bioimpedance.mineral_max)
        ? "over_good"
        : "normal";
    this.cat_weight =
      Number(this.bioimpedance.weight) < Number(this.bioimpedance.weight_min)
        ? "under_bad"
        : Number(this.bioimpedance.weight) >
          Number(this.bioimpedance.weight_max)
        ? "over_bad"
        : "normal";
    this.cat_smm =
      Number(this.bioimpedance.smm_skeletal_muscle_mass) <
      Number(this.bioimpedance.smm_min)
        ? "under_bad"
        : Number(this.bioimpedance.smm_skeletal_muscle_mass) >
          Number(this.bioimpedance.smm_max)
        ? "over_good"
        : "normal";
    this.cat_ffm =
      Number(this.bioimpedance.ffm_fat_free_mass) <
      Number(this.bioimpedance.ffm_min)
        ? "under_bad"
        : Number(this.bioimpedance.ffm_fat_free_mass) >
          Number(this.bioimpedance.ffm_max)
        ? "over_good"
        : "normal";
    this.cat_pbf =
      Number(this.bioimpedance.pbf_percent_body_fat) <
      Number(this.bioimpedance.pbf_min)
        ? "under_bad"
        : Number(this.bioimpedance.pbf_percent_body_fat) >
          Number(this.bioimpedance.pbf_max)
        ? "over_bad"
        : "normal";
    this.cat_fat =
      Number(this.bioimpedance.fat) < Number(this.bioimpedance.fat_min)
        ? "under_bad"
        : Number(this.bioimpedance.fat) > Number(this.bioimpedance.fat_max)
        ? "over_bad"
        : "normal";
    this.cat_bmi =
      Number(this.bioimpedance.bmi) < Number(this.bioimpedance.bmi_min)
        ? "under_bad"
        : Number(this.bioimpedance.bmi) > Number(this.bioimpedance.bmi_max)
        ? "over_bad"
        : "normal";
    this.cat_segmental_lean_ra =
      Number(this.bioimpedance.segmental_lean_ra) <
      Number(this.bioimpedance.segmental_lean_ra_min)
        ? "under_bad"
        : Number(this.bioimpedance.segmental_lean_ra) >
          Number(this.bioimpedance.segmental_lean_ra_max)
        ? "over_good"
        : "normal";
    this.cat_segmental_lean_la =
      Number(this.bioimpedance.segmental_lean_la) <
      Number(this.bioimpedance.segmental_lean_la_min)
        ? "under_bad"
        : Number(this.bioimpedance.segmental_lean_la) >
          Number(this.bioimpedance.segmental_lean_la_max)
        ? "over_good"
        : "normal";
    this.cat_segmental_lean_tr =
      Number(this.bioimpedance.segmental_lean_tr) <
      Number(this.bioimpedance.segmental_lean_tr_min)
        ? "under_bad"
        : Number(this.bioimpedance.segmental_lean_tr) >
          Number(this.bioimpedance.segmental_lean_tr_max)
        ? "over_good"
        : "normal";
    this.cat_segmental_lean_rl =
      Number(this.bioimpedance.segmental_lean_rl) <
      Number(this.bioimpedance.segmental_lean_rl_min)
        ? "under_bad"
        : Number(this.bioimpedance.segmental_lean_rl) >
          Number(this.bioimpedance.segmental_lean_rl_max)
        ? "over_good"
        : "normal";
    this.cat_segmental_lean_ll =
      Number(this.bioimpedance.segmental_lean_ll) <
      Number(this.bioimpedance.segmental_lean_ll_min)
        ? "under_bad"
        : Number(this.bioimpedance.segmental_lean_ll) >
          Number(this.bioimpedance.segmental_lean_ll_max)
        ? "over_good"
        : "normal";
  }
  ...
  getCanvas(this, width: number, height: number) {
    const positions = {
      relLines_old: [
        0.037, 0.0815, 0.1, 0.16, 0.18, 0.1975, 0.215, 0.2325, 0.297, 0.322,
        0.347, 0.372, 0.397, 0.46, 0.482, 0.506, 0.529, 0.553,
      ],
      relLines: [
        0.058, 0.0895, 0.108, 0.168, 0.188, 0.2055, 0.223, 0.2405, 0.305, 0.33,
        0.355, 0.38, 0.401, 0.468, 0.49, 0.514, 0.537, 0.561,
      ],
      relLinesTxt: [
      ...
      relColumn2: [0.22, 0.3, 0.38, 0.61, 0.71, 0.8, 0.9],
      relColumnBar: [0.49],
      relColumnTxt: [0.05],
    };
    ...
    // carregar a imagem
    const newImageBuffer = loadImage(dir + "/folha_result_inbody_s10.jpg")
      .then((image) => {
        ...
        insertText(
          column2[2],
          lines[3],
          `${this.bioimpedance.icw_min} - ${this.bioimpedance.icw_max}`,
          font,
          "std"
        );
        ...
        insertText(
          column2[2],
          lines[4],
          `${this.bioimpedance.ecw_min} - ${this.bioimpedance.ecw_max}`,
          font,
          "std"
        );
        insertText(
          column2[3],
          lines[4],
          this.bioimpedance.tbw_total_body_water,
          font,
          "std"
        );
        ...
        insertText(
          column2[2],
          lines[5],
          `${this.bioimpedance.protein_min} - ${this.bioimpedance.protein_max}`,
          font,
          "std"
        );
        insertText(
          column2[4],
          lines[5],
          this.bioimpedance.slm_soft_lean_mass,
          font,
          "std"
        );
        ...
        insertText(
          column2[2],
          lines[6],
          `${this.bioimpedance.mineral_min} - ${this.bioimpedance.mineral_max}`,
          font,
          "std"
        );
        insertText(
          column2[5],
          lines[6],
          this.bioimpedance.ffm_fat_free_mass,
          font,
          "std"
        );
        ...
        insertText(
          column2[2],
          lines[7],
          `${this.bioimpedance.fat_min} - ${this.bioimpedance.fat_max}`,
          font,
          "std"
        );
        ...
        insertText(
          column2[2],
          lines[8],
          `${this.bioimpedance.weight_min} - ${this.bioimpedance.weight_max}`,
          font,
          "std"
        );
        insertText(
        ...
        insertText(
          column2[1],
          lines[9],
          this.bioimpedance.smm_skeletal_muscle_mass,
          font,
          this.cat_smm
        );
        insertText(
          column2[2],
          lines[9],
          `${this.bioimpedance.smm_min} - ${this.bioimpedance.smm_max}`,
          font,
          "std"
        );
        insertText(
        ...
        insertText(
          column2[1],
          lines[11],
          this.bioimpedance.pbf_percent_body_fat,
          font,
          this.cat_pbf
        );
        insertText(
          column2[2],
          lines[11],
          `${this.bioimpedance.pbf_min} - ${this.bioimpedance.pbf_max}`,
          font,
          "std"
        );
        insertText(
        ...
        insertText(
          column2[2],
          lines[12],
          `${this.bioimpedance.bmi_min} - ${this.bioimpedance.bmi_max}`,
          font,
          "std"
        );
        ...
        insertText(
          column2[1],
          lines[13],
          this.bioimpedance.segmental_lean_la,
          font,
          this.cat_segmental_lean_la
        );
        insertText(
          column2[2],
          lines[13],
          `${this.bioimpedance.segmental_lean_la_min} - ${this.bioimpedance.segmental_lean_la_max}`,
          font,
          "std"
        );
        insertText(
          columnBar[0],
          lines[13],
          this.getBars(this.cat_segmental_lean_la),
          font,
          "std"
        );
        ...
        insertText(
          column2[1],
          lines[15],
          this.bioimpedance.segmental_lean_tr,
          font,
          this.cat_segmental_lean_tr
        );
        insertText(
          column2[2],
          lines[15],
          `${this.bioimpedance.segmental_lean_tr_min} - ${this.bioimpedance.segmental_lean_tr_max}`,
          font,
          "std"
        );
        insertText(
          columnBar[0],
          lines[15],
          this.getBars(this.cat_segmental_lean_tr),
          font,
          "std"
        );
        ...
        insertText(
          column2[1],
          lines[16],
          this.bioimpedance.segmental_lean_rl,
          font,
          this.cat_segmental_lean_rl
        );
        insertText(
          column2[2],
          lines[16],
          `${this.bioimpedance.segmental_lean_rl_min} - ${this.bioimpedance.segmental_lean_rl_max}`,
          font,
          "std"
        );
        insertText(
          columnBar[0],
          lines[16],
          this.getBars(this.cat_segmental_lean_rl),
          font,
          "std"
        );
        ...
        insertText(
          column2[1],
          lines[17],
          this.bioimpedance.segmental_lean_ll,
          font,
          this.cat_segmental_lean_ll
        );
        insertText(
          column2[2],
          lines[17],
          `${this.bioimpedance.segmental_lean_ll_min} - ${this.bioimpedance.segmental_lean_ll_max}`,
          font,
          "std"
        );
        insertText(
          columnBar[0],
          lines[17],
          this.getBars(this.cat_segmental_lean_ll),
          font,
          "std"
        );
